...
- **Before creating, relaunching, or modifying any Codex auto-fix loop, invoke the `codex-autofix-loop` skill first** and follow its parameterizable prompt template, the per-PR state file convention (`.codex-autofix/processed-<PR>.json`), and the auto-cancellation guard (stop the loop when the PR is no longer `OPEN`).
